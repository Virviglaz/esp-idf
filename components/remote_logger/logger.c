...
static void send_message_to_server(void *buf, int max_size)
{
	...
	if (uxQueueMessagesWaiting(msg_queue) == conf->message_queue_size ||
		...
		char *d;
		if (xQueueReceive(msg_queue, &d, 0) == pdPASS)
			free(d);
	}
...
void logger_switch(log_dst_t dst)
{
	...
	log_to_both = dst == LOG_TO_BOTH;
	esp_log_set_vprintf(dst >= LOG_TO_REMOTE ? server_vprintf : vprintf);
}
